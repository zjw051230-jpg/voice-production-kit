...
3. 完整读取 `详细使用手册.md` 和 `API与CCSwitch配置.md`。优先让用户打开 `Seedance API配置工具`，选择项目和 test版/正式版，在遮罩输入框本地添加一份或多份 API；添加、删除和设为当前都会立即写盘，不得输出密钥。`配置Seedance API.ps1` 只作为兼容入口。让用户按文档完成只允许 `5.5 -> deepseek-v4-pro` 的模型映射。
...
看板无审批功能。监控必须等整批满足“成功数 + 失败数 = 总数”后才交给拉回；拉回会再次检查。整批终态且全部成功版本已下载后才显示“成品链接”和“复制到地址”，失败版本不要求成品。复制必须由用户在弹窗输入地址并按提交，禁止双引号、移动、删除和覆盖。
