...
[ordered]@{schema_version=2;repository='https://github.com/zjw051230-jpg/voice-production-kit';installed_version=$manifest.version;update_command="powershell -ExecutionPolicy Bypass -File `"$toolStateRoot\Update-Toolkit.ps1`" -WorkspaceRoot `"$WorkspaceRoot`"";managed_scope=@('Codex official skills','.codex-dashboard','.voice-production-collab','声音本体\01_程序与工具\用户Skills\README.md');user_skill_scope='声音本体\01_程序与工具\用户Skills';official_skill_scope=$skillTarget;protected_scope=@('声音本体\01_程序与工具\用户Skills','项目注册表.json','项目四大目录','.codex task records','API private configuration','素材','提示词','视频','MP3');updated_at=(Get-Date).ToString('o')} | ConvertTo-Json -Depth 6 | Set-Content -Encoding UTF8 -LiteralPath (Join-Path $toolStateRoot 'update-source.json')
