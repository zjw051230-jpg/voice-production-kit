...
新建工作区只允许两个业务顶层目录：`声音本体` 和 `项目文件夹`。

- `声音本体`只存可复用程序、Skills、脚本、模板、任务短索引和运行日志；严禁存放任何素材、完整任务 JSON、提示词、视频、MP3 或成品。
- `项目文件夹`存放每个项目的真实资料。项目内部保留 `01_输入资料`、`02_生产成品`、`03_交付与版本`、`04_管理与记录` 和隐藏 `.codex`。
- 声音本体与项目之间只用 Windows 快捷方式交换当前任务需要的文件。快捷方式必须登记来源、目标、项目、任务和用途；删除快捷方式不得删除真实文件。
- 每条任务在声音本体索引和项目 `.codex` 中只写 20-30 字摘要及路径索引，正文台词、提示词和媒体永远留在项目真实目录。
- 用户只与 `任务理解与分配` 入口 Chat 交互。下属 Chat 的成功、失败、余额不足、权限错误、项目不明、文件缺失和快捷方式失效都必须回传入口；任何下属 Chat 不得把错误留在自身对话后结束。
- 无法唯一确认工作区、项目或任务时必须询问用户，禁止按文件名、最近修改时间或历史对话猜测。

旧项目仍可使用原四大目录和旧路径兼容映射。除用户明确授权外，不迁移、复制、删除、覆盖或重命名旧项目资料。

## 处理工具更新请求

用户说“安装最新配音工具”“更新配音工具”“执行最新更新”或“执行 YYYYMMDD 轮更新”时，不得对工作区执行 Git 同步。固定使用官方仓库 `https://github.com/zjw051230-jpg/voice-production-kit`：最新读取 `releases/latest`，指定轮次读取同名 Release 标签；运行 `Update-Toolkit.ps1 -Revision latest` 或 `-Revision <轮次>`，自动拉取 ZIP 并校验 SHA-256。
