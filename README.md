...
打开协作应用后，可以在“台词生成器”中选择冰川 AI 或自有兼容 OpenAI API，输入剧本和台词要求后返回可复制台词。冰川 AI 走当前登录用户额度，必须在冰川同域页面完成 SSO；自有 API 的 Key 只在当前页面内存中使用，刷新后清空。遇到自有 API 的 CORS 错误时需要改用支持跨域的 API 或配置后端代理。
